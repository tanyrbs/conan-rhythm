...
for split in train-clean-100 dev-clean test-clean; do
  if [[ ! -d "$RAW_ROOT/$split" ]]; then
    echo "[autodl-prepare-train100] missing raw split: $RAW_ROOT/$split" >&2
    exit 1
  fi
done
...
if [[ "$NUM_SHARDS" =~ ^[1-9][0-9]*$ ]] && [[ "$NUM_SHARDS" -gt 1 ]]; then
  ...
  LOG_DIR="logs/prepare_train100_shards"
  rm -rf "$SHARD_ROOT"
  ...
  for (( shard_idx=0; shard_idx<NUM_SHARDS; shard_idx++ )); do
    ...
    rm -rf "$shard_dir"
    echo "[autodl-prepare-train100] launch shard $((shard_idx + 1))/$NUM_SHARDS -> $shard_dir"
    "${PYTHON_RUNNER[@]}" scripts/build_libritts_local_processed_metadata.py \
      --raw_root "$RAW_ROOT" \
      --processed_data_dir "$shard_dir" \
      --config "$CONFIG" \
      --exp_name "build_libritts_local_processed_metadata_train100_shard_${shard_idx}" \
  ...
  "${PYTHON_RUNNER[@]}" scripts/build_libritts_local_processed_metadata.py \
    ...
    --config "$CONFIG" \
    --exp_name "build_libritts_local_processed_metadata_train100" \
    --train_split train-clean-100 \
    --valid_split dev-clean \
    --test_split test-clean \
    --content_source emformer \
