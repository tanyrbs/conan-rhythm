...
class MinimalStreamingDurationHeadV1G(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if (
            float(speech_mask.sum().item()) <= 0.0
            or float(residual_gate.sum().item()) <= 1.0e-6
            or self.local_residual_scale <= 0.0
        ):
            residual = torch.zeros_like(residual_gate)
...
MinimalStreamingDurationWriterV1G = MinimalStreamingDurationHeadV1G
