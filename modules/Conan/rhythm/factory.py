...
def build_projector_config_from_hparams(hparams) -> ProjectorConfig:
    pause_selection_mode = str(
        hparams.get(
            ...
            # Maintained strict-mainline stages rely on pause-topk annealing to
            # create concentrated, phrase-like pause placement. Defaulting to
            # "simple" makes the logged top-k schedule effectively inert.
            'sparse',
        )
        or 'sparse'
    ).strip().lower()
    ...
    return ProjectorConfig(
        ...
        build_render_plan=bool(
            # Keep render-plan sidecars on by default so retimed-target and
...
def build_streaming_rhythm_module_from_hparams(hparams) -> StreamingRhythmModule:
    ...
    return StreamingRhythmModule(
        ...
        pause_source_boundary_weight=float(hparams.get('rhythm_pause_source_boundary_weight', 0.20)),
        min_speech_frames=float(hparams.get('rhythm_projector_min_speech_frames', 1.0)),
        enable_trace_exhaustion_fallback=bool(hparams.get('rhythm_enable_trace_exhaustion_fallback', False)),
