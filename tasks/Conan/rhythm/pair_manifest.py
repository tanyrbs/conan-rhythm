...
def _resolve_prefixed_payload(payload: Any, prefix: str | None) -> Any:
    if prefix is None or not isinstance(payload, dict):
        return payload
    if prefix in payload:
        return payload[prefix]
    for key in ("splits", "datasets", "manifests"):
        nested = payload.get(key)
        if isinstance(nested, dict) and prefix in nested:
...
def _normalize_manifest_entries(raw_entries: Any) -> list[dict[str, Any]]:
    ...
    for entry in raw_entries:
        ...
        entry_identity_anchor = bool(entry.get("identity_anchor", False))
        ...
        include_self = bool(entry.get("include_self", False)) or entry_identity_anchor
        ...
        if include_self and all(str(ref["ref_item_name"]) != source_name for ref in expanded_refs):
            expanded_refs = [
                {
                    "ref_item_name": source_name,
                    "pair_label": "self",
                    "identity_anchor": bool(entry_identity_anchor),
                }
            ] + expanded_refs
        for rank, ref_entry in enumerate(expanded_refs):
            ref_item_name = str(ref_entry["ref_item_name"])
            is_self_pair = ref_item_name == source_name
            pair_label = (
            ...
            pair_rank = int(
                ref_entry.get(
                    ...
                    ref_entry.get(
                        "group_slot",
                        entry.get("pair_rank", rank),
                    ),
                )
            )
            group_slot = int(ref_entry.get("group_slot", pair_rank))
            normalized.append(
                {
                    "source_item_name": source_name,
                    "ref_item_name": ref_item_name,
                    "group_id": group_id,
                    "pair_rank": pair_rank,
                    "group_slot": group_slot,
                    "pair_label": None if pair_label in {None, ""} else str(pair_label),
                    "pair_weight": float(ref_entry.get("pair_weight", entry.get("pair_weight", 1.0)) or 1.0),
                    "identity_anchor": bool(
                        ref_entry.get("identity_anchor", False)
                        or (entry_identity_anchor and is_self_pair)
                    ),
                }
            )
    return normalized
...
def load_pair_manifest(path: str | Path, *, prefix: str | None = None) -> list[dict[str, Any]]:
    ...
    payload = _resolve_prefixed_payload(payload, prefix)
